...
Switches and parameters can be configured to be used interchangeably when running the program
...
Switches are required by default, you can make any of them optional using the syntax:
`[{name}={value}]` (e.g.: `[name=Ringo]`)
...
Let's try a more complete example, a program that can receive your coffee order, note that
`topping` is a variadic switch/parameter and is marked as optional (surrounded by `[]`):
...
**Note**: If you want to use the `@` symbol on any of your switch values or parameters, you can
escape it as `@@`:
...
directory or our [wiki](https://github.com/dongryphon/switchit/wiki) for more information and
API docs.
...
See also the list of [contributors](https://github.com/dongryphon/switchit/contributors) who
